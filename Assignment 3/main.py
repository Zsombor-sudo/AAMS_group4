...
def checkApple(x , y):
    neighbor_apples = []
    for apple in apples:
        apple_x,apple_y = apple.getCoor()
        if bool(abs(apple_x-x)<1.1) ^ bool(abs(apple_y-y)==1.1):
            neighbor_apples.append(apple.getId)
    return neighbor_apples
...
def step_agent(agent: ObjectBase):
    ...
    if not curr_state["moving"]:
        ...
        if checkApple(agent.state[0],agent.state[1]) != []:
            action = "collect"
