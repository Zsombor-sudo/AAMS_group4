...
@register_behavior("diff", "follow_leader_line")
def beh_diff_followLeaderLine(
...
def followLeader(
    ...
    diff_radian = WrapToPi(radian - state[2, 0])
    linear = (max_vel[0, 0] / 2) * np.cos(diff_radian)

    if neighbor_list is not None:
        for neighbor in neighbor_list:
            n_distance, n_radian = relative_position(state, neighbor.state)

            diff_radian += -sin(n_radian) * (math.pi/2) * pow(math.e, -pow(n_distance/1,2))

    WrapToPi(diff_radian)
