...
def calculate_R(robot):
    ...
    metrics.update(robot.id,radial_error[0])
...
for episode in range(num_episodes):
    ...
    for step in range(num_steps):
        ...
        for i, robot in enumerate(robots):
            actionStep = circle_following_action(states[i], Q_tables[i])
            metrics.updateSpeed(robot.id,actionStep[0])
            metrics.updateAngular(robot.id,actionStep[1])
            metrics.print(robot.id)
